...
const rxScript = /([^\/]+?)(?:\.(js|coffee))?$/i
function scriptTitle (s) {
  const m = rxScript.exec(s)
  return m && m[1]
}
const rxPM2 = /^pm2(?:\/lib\/ProcessContainerFork\.js-|.*:\s)(.+)$/i
function appId (fallback) {
  fallback = fallback || process.argv[1]
  if (process.title === 'node') {
    return scriptTitle(process.argv[1] || fallback)
  }
  let m = rxPM2.exec(process.title)
  return scriptTitle((m && m[1]) || fallback)
}
